...
def test_regression():
    ...
    assert all(
        df_project.loc[
            ...
            & (df_project[DbColumns.COUNTRY] == ECUADOR),
            DbColumns.VALUE,
        ]
        == np.array([7, 8, 9])
    )
...
def test_baseline_forward_fill():
    ...
    assert all(
        df_project.loc[
            (df_project[DbColumns.SECTOR] == ONE_A)
            & (df_project[DbColumns.GAS] == N2O)
            & (df_project[DbColumns.COUNTRY] == USA),
            DbColumns.VALUE,
        ]
        == 4.37
    )
...
def test_data_cleaning():
    ...
    assert (
        proj_edgar.data.loc[
            ...
            & (proj_edgar.data[DbColumns.YEAR] == 2018),
            DbColumns.VALUE,
        ].item()
        == 4.37
    )
