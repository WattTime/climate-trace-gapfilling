...
def _sem_open(name, value=None):
    ...
    if value is None:
        ...
        handle = pthread.sem_open(
            ctypes.c_char_p(name), SEM_OFLAG, SEM_PERM, ctypes.c_int(value)
        )
...
def _sem_timedwait(handle, timeout):
    ...
    if sys.platform != "darwin":
        ...
        tv_nsec = int(1e9 * (t_start - tv_sec) + 0.5)
        ...
        if self.kind == RECURSIVE_MUTEX:
            ...
            if sys.platform == "darwin":
                ...
                if self.maxvalue == 1:
                    ...
                    warnings.warn(
                        "semaphore are broken on OSX, release might "
