...
cdf_pdf_cases = [
    CdfCase(*case, general_atol, general_rtol)
    for case in itertools.product(cp_q, cp_k, cp_nu)
]
...
def moment_mp(m, k, nu):
    ...
    res = quad(
        whole, [0, inf], [0, inf], [-inf, inf], method="gauss-legendre", maxdegree=10
    )
    return res
...
def run(case, run_lambda, set_key, index=0, total_cases=0):
    ...
    print(
        f"Finished {index + 1}/{total_cases} in batch. "
...
def run_cases(cases, run_lambda, set_key):
    ...
    job_arg = [
        (case, run_lambda, set_key, index, len(cases))
        for index, case in enumerate(cases)
        if not result_exists(set_key, case)
    ]

    print(
        f"{len(cases) - len(job_arg)}/{len(cases)} cases won't be "
...
def main():
    ...
    print(
        f"Running 1st batch ({len(cdf_pdf_cases)} PDF cases). "
        f"These take about 30s each."
    )
    run_cases(cdf_pdf_cases, run_pdf, "pdf_data")

    print(
        f"Running 2nd batch ({len(cdf_pdf_cases)} CDF cases). "
        f"These take about 30s each."
    )
    run_cases(cdf_pdf_cases, run_cdf, "cdf_data")

    print(
        f"Running 3rd batch ({len(moment_cases)} moment cases). "
