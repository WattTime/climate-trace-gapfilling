...
class TestInterp1D:
    ...
    def test_init(self):
        ...
        assert_equal(
            interp1d(self.x10, self.y10, fill_value=(1.0, 2.0)).fill_value, (1.0, 2.0)
        )
            ...
            for dty in dt_rc:
                ...
                for dtn in dt_r:
                    ...
                    for kind in spline_kinds:
                        ...
                        assert_allclose(
                            f(xnew), y, atol=1e-7, err_msg="%s, %s %s" % (dtx, dty, dtn)
                        )
                    ...
                    for i in range(2):
                        for j in range(3):
                            ...
                            if rr.size > 0:
                                ...
                                assert_allclose(
                                    (val - y) / cmpval, 0, atol=1e-7, err_msg=msg
                                )
...
class TestBPolyCalculus:
    def test_derivative(self):
        ...
        assert_allclose(
            [bp(0.4, nu=1), bp(0.4, nu=2), bp(0.4, nu=3)], [-6 * (1 - 0.4), 6.0, 0.0]
        )
...
class TestRegularGridInterpolator:
    ...
    def test_nearest(self):
        ...
        sample = np.asarray([0.1, 0.1, 0.9, 0.9])
        wanted = 1100.0
        assert_array_almost_equal(interp(sample), wanted)
        sample = np.asarray([0.1, 0.1, 0.1, 0.1])
        wanted = 0.0
        assert_array_almost_equal(interp(sample), wanted)
        sample = np.asarray([0.0, 0.0, 0.0, 0.0])
        wanted = 0.0
        assert_array_almost_equal(interp(sample), wanted)
        sample = np.asarray([1.0, 1.0, 1.0, 1.0])
        wanted = 1111.0
        assert_array_almost_equal(interp(sample), wanted)
...
class TestInterpN:
    ...
    def test_nonscalar_values(self):
        ...
        for method in ["nearest", "linear"]:
            ...
            vs = [
                interpn(
                    points, values[..., j], sample, method=method, bounds_error=False
                )
                for j in range(6)
            ]
    ...
    def test_complex(self):
        ...
        assert_warns(
            np.ComplexWarning, interpn, points, values, sample, method="splinef2d"
        )
