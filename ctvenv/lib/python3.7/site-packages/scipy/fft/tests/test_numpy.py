...
class TestFFT1D:
    ...
    def test_identity(self):
        ...
        for i in range(1, maxlen):
            ...
            assert_array_almost_equal(
                fft.irfft(fft.rfft(xr[0:i]), i), xr[0:i], decimal=12
            )
    ...
    def test_fftn(self):
        ...
        assert_array_almost_equal(
            expect / np.sqrt(30 * 20 * 10), fft.fftn(x, norm="ortho")
        )
    ...
    def test_ifftn(self):
        ...
        assert_array_almost_equal(
            fft.ifftn(x) * np.sqrt(30 * 20 * 10), fft.ifftn(x, norm="ortho")
        )
    ...
    def test_rfft(self):
        ...
        for n in [x.size, 2 * x.size]:
            ...
            assert_array_almost_equal(
                fft.rfft(x, n=n) / np.sqrt(n), fft.rfft(x, n=n, norm="ortho")
            )
    ...
    def test_rfftn(self):
        ...
        assert_array_almost_equal(
            expect / np.sqrt(30 * 20 * 10), fft.rfftn(x, norm="ortho")
        )
    ...
    def test_ihfft2(self):
        ...
        assert_array_almost_equal(
            expect * np.sqrt(30 * 20), fft.ihfft2(x, norm="ortho")
        )
    ...
    def test_ihfftn(self):
        ...
        assert_array_almost_equal(
            expect * np.sqrt(30 * 20 * 10), fft.ihfftn(x, norm="ortho")
        )
        assert_array_almost_equal(
    ...
    @pytest.mark.parametrize(
        "op", [fft.fftn, fft.ifftn, fft.rfftn, fft.irfftn, fft.hfftn, fft.ihfftn]
    )
...
@pytest.mark.parametrize(
    "dtype",
    [np.float32, np.float64, np.longfloat, np.complex64, np.complex128, np.longcomplex],
)
