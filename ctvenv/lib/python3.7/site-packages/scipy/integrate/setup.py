...
def configuration(parent_package="", top_path=None):
    ...
    cfg = combine_dict(
        lapack_opt, include_dirs=include_dirs, libraries=["quadpack", "mach"]
    )
    ...
    cfg = combine_dict(lapack_opt, numpy_nodepr_api, libraries=["lsoda", "mach"])
    config.add_extension(
        "_odepack", sources=["_odepackmodule.c"], depends=(lsoda_src + mach_src), **cfg
    )

    # vode
    cfg = combine_dict(lapack_opt, libraries=["vode"])
    ext = config.add_extension(
        "vode", sources=["vode.pyf"], depends=vode_src, f2py_options=f2py_options, **cfg
    )
