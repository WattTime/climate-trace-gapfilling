...
def transform_years_to_columns(df):
    transformed_df = df.pivot(
        index=["Sector", "Country", "Data source", "Gas", "Unit"],
        columns="year",
        values="emission_quantity",
    ).reset_index()
...
def generate_carbon_equivalencies(dh, df, co2e_to_compute=100):
    ...
    merged_df[COMP_YEARS] = merged_df[COMP_YEARS].multiply(
        merged_df[col_to_mult], axis=0
    )
    # Sum up the multiplied numbers and return them
    co2e_vals = merged_df.groupby(
        ["Country", "Sector", "Data source", "Unit"], as_index=False
    )[COMP_YEARS].sum()
...
def add_all_gas_rows(df):
    ...
    multi_index_all_years = pd.MultiIndex.from_product(
        [countries, sectors, gases], names=multi_ind_col_list
    )
    ...
    df = (
        ...
        .reindex(multi_index_all_years, fill_value=np.nan)
        .reset_index()
    )
...
def add_carbon_eq_column(df):
    ...
    cem_list = [
        cem_str[cem_gases[g]] if g in cem_gases.keys() else ""
        for g in df["emitted_product_formula"]
    ]
...
def add_iso_value(df):
    df["producing_entity_id"] = [
        get_iso3_code(pen) for pen in df["producing_entity_name"]
    ]
