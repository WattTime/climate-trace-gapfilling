...
class ProjectEdgarData:
    ...
    def clean(self):
        ...
        index_date_range = np.arange(
            self.start_training_year, self.end_training_year + 1
        )
        ...
        self.full_df = (
            self.data[self.data[DbColumns.COUNT] == len(index_date_range)]
            .sort_values(by=self.group_list_with_year)
            .reset_index(drop=True)
        )

        self.some_missing_df = (
            self.data[
                (self.data[DbColumns.COUNT] > 0)
                & (self.data[DbColumns.COUNT] < len(index_date_range))
            ]
            .sort_values(by=self.group_list_with_year, ignore_index=True)
            .reset_index(drop=True)
        )

        self.all_missing_df = (
            self.data[self.data[DbColumns.COUNT] == 0]
            .sort_values(by=self.group_list_with_year, ignore_index=True)
            .reset_index(drop=True)
        )
    ...
    def _reindex(self, index_date_range):
        ...
        self.multi_index_all_years = pd.MultiIndex.from_product(
            [self.country_list, self.sector_list, self.gases_list, index_date_range],
            names=self.group_list_with_year,
        )

        print(
            f"Number of country-sector-gas combinations: {len(self.multi_index_all_years)}"
        )

        self.data = (
            self.data.set_index(self.group_list_with_year)
            .reindex(self.multi_index_all_years, fill_value=np.nan)
            .reset_index()
        )
    ...
    def project(self):
        ...
        df_regression_full = self._apply_regression(
            self.full_df[
                self.full_df[DbColumns.SECTOR].isin(self.sectors_to_use_regression)
            ].copy()
        )

        # Apply forward fill to appropriate sectors for all partial or full data
        df_no_missing = pd.concat(
            [self.full_df, self.some_missing_df], ignore_index=True
        )
        df_baseline_results = self._apply_baseline_forward_fill(
            df_no_missing[
                ~df_no_missing[DbColumns.SECTOR].isin(self.sectors_to_use_regression)
            ].copy()
        )
