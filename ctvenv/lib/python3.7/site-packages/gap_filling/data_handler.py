...
class DataHandler:
    ...
    def load_data(
        ...
        if gas is None:
            curs.execute(
                "SELECT original_inventory_sector, producing_entity_name, reporting_entity, "
        ...
        return parse_and_format_query_data(
            pd.DataFrame(data=np.array(curs.fetchall()), columns=np.array(colnames)),
            years_to_columns=years_to_columns,
    ...
    def write_data(self, data_to_insert, rows_type=None):
        ...
        if rows_type == "climate-trace":
            ...
            insert_str = (
                "INSERT INTO ermin (original_inventory_sector, producing_entity_name, producing_entity_id, "
        ...
        for dti in data_to_insert.iterrows():
            data_row_dict = {
                k: dti[1][INSERT_MAPPING[k]] for k in INSERT_MAPPING.keys()
            }
